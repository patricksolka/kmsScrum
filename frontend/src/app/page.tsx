...
export default function Home() {
  ...
  const handleSaveEdit = () => {
    ...
    updatedTasks[editingIndex!] = { task: editTaskInput, description: editDescInput };
  ...
  return (
      <>
        <div id="todo-app" className="container mx-auto p-4 mt-4 md:p-6 lg:p-8 xl:p-10">
          ...
          <form onSubmit={handleSubmit} className="flex justify-center">
            ...
            />
            <button type="submit"
                    className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ml-4">Add
            </button>
          </form>
        </div>
        ...
        <ul id="ToDoListe" className="container mx-auto list-none m-0 p-0">
          {tasks.map((task, index) => (
              <li key={index} className="border border-gray-300 rounded-lg p-4 mb-4 flex justify-between items-center">
                <div>
                  <strong className="block text-lg font-semibold">{task.task}</strong>
                  <small className="block text-sm text-gray-500">{task.description}</small>
                </div>
